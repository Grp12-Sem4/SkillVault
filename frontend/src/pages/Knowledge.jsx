...
export default function Knowledge() {
	...
	const decayKnowledgeMutation = useMutation({
		mutationFn: async (topicId) => {
			const response = await api.put(`/knowledge/${topicId}/decay`);
			return response.data;
		},
		onSuccess: () => {
	...
	const subjectOptions = useMemo(() => {
		...
		return [
			"ALL",
			...Array.from(subjects).sort((left, right) => left.localeCompare(right)),
		];
	}, [allTopics]);
	...
	const knowledgeSummary = useMemo(() => {
		...
		const averageMastery = totalTopics
			? Math.round(
					allTopics.reduce(
						(sum, topic) => sum + (Number(topic.masteryLevel) || 0),
						0,
					) / totalTopics,
				)
			: 0;
				...
				<div>
					...
					>
						...
						<p className="app-subtle-text" style={{ margin: 0 }}>
							Track all of your learning topics, spot revision risk early, and
							refresh topics that need attention.
						</p>
					</header>
				</div>
				...
				<section className="app-panel" style={panelStyle}>
					<div className="app-section-header">
						<div>
							...
							<p className="app-subtle-text" style={{ margin: "0.35rem 0 0" }}>
								A quick snapshot of the current user’s topic library and
								revision risk.
							</p>
						</div>
					</div>
					...
					{isAllTopicsError ? (
						<p className="app-feedback-error">
							{getErrorMessage(
								allTopicsError,
								"Unable to load your knowledge summary.",
							)}
						</p>
					) : null}
							...
							<div className="app-card-grid-compact">
								<article className="app-stat-card">
									...
									<p className="app-stat-value">
										{knowledgeSummary.totalTopics}
									</p>
									<p className="app-subtle-text">
										Everything in your personal knowledge vault.
									</p>
								</article>
								...
								<article className="app-stat-card">
									...
									<p className="app-stat-value">
										{knowledgeSummary.revisionCount}
									</p>
									<p className="app-subtle-text">
										Topics already flagged for another review pass.
									</p>
								</article>
								...
								<article className="app-stat-card">
									...
									<p className="app-stat-value">
										{knowledgeSummary.averageMastery}%
									</p>
									<p className="app-subtle-text">
										Average stored mastery across all your topics.
									</p>
								</article>
								...
								<article className="app-stat-card">
									...
									<p className="app-stat-value">
										{knowledgeSummary.distinctSubjects}
									</p>
									<p className="app-subtle-text">
										Distinct subject areas currently represented.
									</p>
								</article>
							</div>
							...
							>
								...
								>
									<div>
										...
										>
											Simple buckets to show how healthy your knowledge
									...
									<div className="app-chip-row">
										<span className="app-chip app-chip-active">
											High: {knowledgeSummary.masteryDistribution.high}
										</span>
										<span className="app-chip app-chip-pending">
											Medium: {knowledgeSummary.masteryDistribution.medium}
										</span>
										<span className="app-chip app-chip-danger">
											Low: {knowledgeSummary.masteryDistribution.low}
										</span>
									</div>
								</article>
								...
								>
									...
									<div className="app-progress">
										<div
											className={getMasteryBarClassName(
												knowledgeSummary.averageMastery,
											)}
											style={{
												width: `${Math.max(0, Math.min(100, knowledgeSummary.averageMastery))}%`,
											}}
										/>
									</div>

									<p className="app-subtle-text">
										Portfolio mastery indicator:{" "}
										{knowledgeSummary.averageMastery}%
									</p>
								</article>
							</div>
						...
						{createTopicMutation.isError ? (
							<p className="app-feedback-error">
								{getErrorMessage(
									...
									"Unable to create knowledge topic right now.",
								)}
							</p>
						) : null}
				...
				<section className="app-panel" style={panelStyle}>
					<div className="app-section-header">
						<div>
							...
							<p className="app-subtle-text" style={{ margin: "0.35rem 0 0" }}>
								Your full authenticated topic list from{" "}
								<code>/api/knowledge</code>.
							</p>
						</div>
						...
						>
							{filteredTopics.map((topic) => {
								...
								const isReviewingCurrentTopic =
									reviewTopicMutation.isPending &&
									reviewTopicMutation.variables === topic.id;
									...
									>
										...
										<div style={{ display: "grid", gap: "0.45rem" }}>
											...
											<div className="app-progress">
												<div
													...
													style={{
														width: `${Math.max(0, Math.min(100, masteryLevel))}%`,
													}}
												/>
											</div>
										</div>
										...
										<div className="app-chip-row">
											<span className="app-chip">
												Decay Rate: {Number(topic.decayRate ?? 0).toFixed(2)}
											</span>
											<span className="app-chip">
												Last Reviewed: {formatDate(topic.lastReviewed)}
											</span>
										</div>

										<div className="app-detail-list">
											<span>
												Last decay check: {formatDate(topic.lastDecayCheck)}
											</span>
				...
				<section className="app-panel" style={panelStyle}>
					...
					{isRevisionTopicsError ? (
						<p className="app-feedback-error">
							{getErrorMessage(
								revisionTopicsError,
								"Unable to load topics needing revision.",
							)}
						</p>
					) : null}

					{!isRevisionTopicsLoading &&
					!isRevisionTopicsError &&
									...
									>
										...
										>
											<button
												...
												style={buttonStyle}
												onClick={() =>
													reviewTopicMutation.mutate(topic.topicId)
												}
												disabled={
													reviewTopicMutation.isPending &&
													reviewTopicMutation.variables === topic.topicId
												}
											>
												{reviewTopicMutation.isPending &&
												reviewTopicMutation.variables === topic.topicId
											...
											<button
												...
												onClick={() =>
													decayKnowledgeMutation.mutate(topic.topicId)
												}
												disabled={
													decayKnowledgeMutation.isPending &&
													decayKnowledgeMutation.variables === topic.topicId
												}
											>
												Simulate Decay
											</button>
					...
					{reviewTopicMutation.isError ? (
						<p className="app-feedback-error" style={{ marginTop: "1rem" }}>
							{getErrorMessage(
								reviewTopicMutation.error,
								"Unable to mark the topic as reviewed.",
							)}
						</p>
					) : null}
				</section>
