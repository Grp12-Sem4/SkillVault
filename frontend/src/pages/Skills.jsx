...
export default function Skills() {
	...
	const practiceSkillMutation = useMutation({
		mutationFn: async (skillId) => {
			const response = await api.put(`/api/skills/${skillId}/practice?hours=1.0`);
			return response.data;
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: SKILLS_QUERY_KEY });
	...
	const decaySkillMutation = useMutation({
		mutationFn: async (skillId) => {
			const response = await api.put(`/api/skills/${skillId}/decay`);
			return response.data;
		},
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: SKILLS_QUERY_KEY });
			...
			<section className="app-layout" style={layoutStyle}>
				<div>
					...
					>
						...
						<p className="app-subtle-text" style={{ margin: 0 }}>
							Add new skills, see your current progress, and log practice
							sessions directly against your profile.
						</p>
					</header>
				</div>
				...
				<section className="app-panel" style={panelStyle}>
					...
					<form onSubmit={handleSubmit} style={formGridStyle}>
						...
						<div className="field-group">
							<label className="field-label" htmlFor="creditValue">
								Credit Value
							</label>
							<input
								className="app-input"
						...
						<div className="field-group">
							<label className="field-label" htmlFor="difficultyLevel">
								Difficulty
							</label>
							<select
								className="app-input"
						...
						{createSkillMutation.isError ? (
							<p className="app-feedback-error" style={fullWidthStyle}>
								{getErrorMessage(
									...
									"Unable to create skill right now.",
								)}
							</p>
						) : null}
				...
				<section className="app-panel" style={panelStyle}>
						...
						<div style={skillGridStyle}>
							{skills.map((skill) => {
								const isPracticingCurrentSkill =
									practiceSkillMutation.isPending &&
									practiceSkillMutation.variables === skill.id;
								...
								return (
									...
									>
										...
										<div style={metaRowStyle}>
											...
											<span style={badgeStyle}>
												Rating: {skill.averageRating?.toFixed(1) ?? 0} (
												{skill.ratingCount ?? 0})
											</span>
											<span style={scoreBadgeStyle}>
												Score:{" "}
												{typeof skill.skillScore === "number"
										...
										{skill.type !== "OFFERED" && (
											...
											>
												...
												<button
													...
													onClick={() => decaySkillMutation.mutate(skill.id)}
													disabled={
														decaySkillMutation.isPending &&
														decaySkillMutation.variables === skill.id
													}
												>
													Simulate Decay
												</button>
					...
					{practiceSkillMutation.isError ? (
						<p className="app-feedback-error" style={{ marginTop: "1rem" }}>
							{getErrorMessage(
								practiceSkillMutation.error,
								"Unable to log practice right now.",
							)}
						</p>
					) : null}
				</section>
